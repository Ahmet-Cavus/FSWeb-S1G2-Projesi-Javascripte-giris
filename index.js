...
Aşağıdakileri yap:   
   ...
   İPUCU: fonksiyon oluşturmaya gerek yok
*/
const birinciDeger = 5;
const ikinciDeger = 8;

if(birinciDeger > ikinciDeger) {
  console.log("Birinci değer daha büyüktür.")
}else{
  console.log("İkinci değer daha büyüktür.")
}
/*
...
function kopeginYasi(kopekKacYasında){
  console.log(kopekKacYasında * 7);
}
kopeginYasi(3);
...
const rastgeleSayi = bilgisayarinSecimi();

let rastgeleSecim;

if(rastgeleSayi < 0.33){
  rastgeleSecim = "Taş";
}else if(rastgeleSayi > 0.33 && rastgeleSayi < 0.66){
  rastgeleSecim = "Kağıt";
}else{
  rastgeleSecim = "Makas";
}
console.log(rastgeleSecim);
...
console.log(oyun("Makas",rastgeleSecim));
...
function milDonusturucu(kmDegeri){
  let milDegeri = kmDegeri * 0.621371;
  return milDegeri;
}
milDonusturucu(10);
...
function cocukSarkisi(sayi){
  for(let i=sayi; i>=1; i--){
    console.log(`${i} küçük maymun yatakta zıplamış, biri düşüp başını çarpmış, Anne doktoru aramış, Doktor çok kmış: Bir daha yatakta zıplamak yok!`)
  }
